...
impl Contract {
    pub fn dr_validate(&self, data_request: &NewDataRequestArgs) {
        ...
        assert!((data_request.description.is_none() && data_request.sources.as_ref().unwrap_or(&vec![]).len() as u8 != 0) || data_request.description.is_some(), "Description should be filled when no sources are given");
        assert!(data_request.sources.as_ref().unwrap_or(&vec![]).len() as u8 <= MAX_SOURCES, "Too many sources provided, max sources is: {}", MAX_SOURCES);
